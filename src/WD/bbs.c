...
//    DL_func("SO/votebrd.so:va_do_voteboardreply",fhdr);
  ...
  {
    char *choose[4]={"fF.¬ÝªO","mM.§@ªÌ«H½c","bB.¨âªÌ¬Ò¬O","qQ.¨ú®ø"};
...
int /*¤å³¹Ãþ§O by hialan 3.21.2002*/
b_load_class(bname)
  char *bname;
{
      FILE *prefixfile;
      char chartemp[PREFIXLEN],buf[MAXPATHLEN];
...
int
make_cold(board,save_title,money, fpath)
  char *board;
  char *save_title;
  int money;
  char *fpath;  //Âà¸ô¤å³¹
{
  ...
  if(cold == 9)
  {
    FILE *fcoldin,*fcoldout;
    char coldfile[30];
    time_t now = time(0);

    do_copy_post("ColdKing", fpath, FILE_MARKED); // Âà¿ý¤å³¹
	  
    strcpy(coldfile,BBSHOME"/etc/cold");

    if((fcoldin = fopen(coldfile,"r")) != NULL)
    {
      char wordd[41][200];
      int i,j;
      ...
      for(i=0;i<41;i++)
      {
        if(feof(fcoldin))break;
        fgets(wordd[i],199,fcoldin);
      }

      fclose(fcoldin);

      if((fcoldout = fopen(coldfile,"w")) != NULL)
      {
        fprintf(fcoldout,"%s",wordd[0]);
        fprintf(fcoldout,"\033[1;46m¤¤¼ú¤H:\033[m%s \033[1;46m¬ÝªO:\033[0mboard://%s \033[0;1;46m®É¶¡:\033[m%s\033[1;44m¤å³¹¼ÐÃD:\033[m%s \033[1;44m½ZÄw:\033[m%d\n\n",cuser.userid, board,ctime(&now), save_title, money);

        if(i > 0)
          for(j=1;j<i-1;j++)
            fprintf(fcoldout,"%s",wordd[j]);
        ...
        if(cold == 9)
	{
	  ...
	  prints ("\n\n              ±zªº¤å³¹¤w¸g³QÂà¿ý¦Ü ColdKing ¬ÝªO¸Ì!!");
        ...
        if (money >= 100 || spendtime <=3)
        {
          ...
          time_t now = time (0);
          fileheader mhdr;
          char genbuf1[MAXPATHLEN], fpath1[STRLEN];
...
read_post (ent, fhdr, direct)
  ...
  char *direct;
{
  char genbuf[256], buf[MAXPATHLEN];
  int more_result;
...
void
score_note(prompt, fhdr, direct)	//±ÀÂË¥[¤À!! ¾Ç ptt by hialan
  fileheader * fhdr;
  char *direct, *prompt;
{
  ...
  char *direct;
{
  ...
  {
    fhdr->score++;
    score_note("±ÀÂË¤º®e(¤£¿é¤J¸õ¹L): ", fhdr, direct);
  }
  else if(buf[0] == '2' && fhdr->score > -9)
    fhdr->score--;
  else if(buf[0] == '3')	// ±ÀÂË ....:) ¾Ç ptt by hialan
  {
    score_note("±ÀÂË¤º®e: ", fhdr, direct);
    return RC_DRAW;
  }
  else if(buf[0] != 'q')
  {
...
board_edit ()
{
  ...
  if (currmode & MODE_BOARD)
  {
    char genbuf[BTLEN], buf[MAXPATHLEN], ans;
...
{
  KEY_TAB, board_digest, 0,"¶i¤J/°h¥X ¤åºK",
  'b', b_notes,          0,"¬Ý¶iª©µe­±",
  'c', cite,             0,"¦¬¿ýºëµØ",
  'r', read_post,        0,"¾\\Åª¤å³¹",
  'z', man,         	 0,"¶i¤JºëµØ°Ï",
  'D', del_range,	 0,"¤j D ¬å«H",
  Ctrl ('S'), save_mail, 0,"¦s¤J«H½c",
  'E', edit_post,        0,"­×§ï¤å³¹",
  'T', edit_title,       0,"­×§ï¼ÐÃD",
  's', do_select,        0,"¿ï¾Ü¬ÝªO",
  'B', board_edit,       0,"¬ÝªO½s¿è",
  't', add_tag,          0,"¼Ð°O¤å³¹",
  Ctrl ('D'), del_tag,   0,"§R°£¼Ð°O¤å³¹",
  'x', cross_post,       0,"Âà¶K",
  'g', good_post,        0,"¦¬¨ì¤åºK¤¤",
  'y', reply_post,       0,"¦^ÂÐ¤å³¹",
  'd', del_post,         0,"§R°£¤å³¹",
  'm', mark,             0,"Mark ¤å³¹",
  'X', refusemark,       0,"¤å³¹¥[±K",
  Ctrl ('P'), do_post,   0,"µoªí¤å³¹",
  'C', gem_tag,          0,"¦¬¿ý¼Ð°O¤å³¹",
  Ctrl ('C'), Cite_posts,0,"ª½±µ¦¬¿ý¤å³¹¦ÜºëµØ°Ï",
  '%', score,		 0,"¤å³¹µû¤À",
  'v', v_board,		 0,"ªO¤ºvªO",
  'w', write_msg,	 PERM_LOGINOK,"ªO¤º¥á¤ô²y",
  'F', post_mail,	 PERM_FORWARD,"±N¤å³¹±H¦^ Internet ¶l½c",
  'U', post_mail_uncode, PERM_FORWARD,"±N¤å³¹ uncode «á±H¦^ Internet ¶l½c",
  Ctrl ('Q'), post_query,0,"ªO¤º q ¤H",
  'V', post_vote,	 0,"°Ñ»P§ë²¼",
  'R', post_b_results,	 0,"¬Ý§ë²¼µ²ªG",
  '\0', NULL, 0, NULL
};
...
//  if(check_money(1,GOLD)) return;
  ...
  do
  {
    char *choose[3]={"sS.Àx¦s","eE.­«·s¨Ó¹L","qQ.¨ú®ø"};
