...
talk_mail2user(void)
{
  ...
  strcpy(mhdr.title, "¼ö½u\033[37;41m°O¿ý\033[m");
...
#define LB_BL 3 //blink °{Ã{
#define LB_UL 4 //underline ©³½u

int
save_lightbar_color(color)
  char *color;
{
  int i, unum;
  
  if(unum = do_getuser(cuser.userid, &xuser))
  {
    for(i=0;i<5;i++)
  ...
  char *color;
{
  ...
  if(cuser.lightbar[LB_LT] == 0)
    sprintf(color, "\033[m");
  else if(cuser.lightbar[LB_LT] == 1)
    sprintf(color, "\033[1m");

  if(cuser.lightbar[LB_BG])
  {
    sprintf(buf,"\033[4%dm",cuser.lightbar[LB_BG]);
    strcat(color, buf);
  }

  if(cuser.lightbar[LB_BL])
    strcat(color,"\033[5m");

  if(cuser.lightbar[LB_UL])
    strcat(color,"\033[4m");
  
  /*¤å¦rÃC¦â*/
  sprintf(buf,"\033[3%dm",cuser.lightbar[LB_WD]);
  strcat(color,buf);
...
change_lightbar()
{
  int i=0;
  char ch[5];
  char *ques[5]={"©³¦â ","¦r¦â ","«G«× ","°{Ã{ ","©³½u "}; 
  char *color[8]={"00.¶Â¦â","11.¬õ¦â","22.ºñ¦â","33.¶À¦â","44.ÂÅ¦â","55.µµ¦â","66.«C¦â","77.¥Õ¦â"};
  char *underline[2]={"00.µL©³½u","11.©³½u"};
  char *blink[2]={"00.¤£°{Ã{","11.°{Ã{"};
  char *light[3]={"00.§C«G«×","11.°ª«G«×","22.¤£³]©w"};
  char **choose[5];
  char def[5] = {'4','7','1','0','0'};
  int many[5] = {8, 8, 3, 2, 2};
...
*/  
  ...
  for(i=0;i<5;i++)
  {
    ch[i] = getans2(i,0,ques[i],choose[i],many[i],def[i]);
    ch[i] = ch[i] - '0';
  }

/*
  if (win_select("¦Û­q¥ú´Î¼Ë¦¡", "¬O§_­n¨Ï¥Î¦Û­q¥ú´Î¼Ë¦¡", 0, 2, 1, 'n') == 'n')
    return 0;
*/
