...
while(-1){
  ...
  i=bet*2-bet/100*tax;
  switch(win){
    case 2:pressanykey_old("[1;33m§AÄ¹¤F %d !!.. :D[44m",i-bet);inmoney(i);break;
    case 1:pressanykey_old("[1;36m¥­¤â!!..[44m");inmoney(bet);break;
    case 0:pressanykey_old("[1;31m§A¿é¤F³á.. :))[44m" );break;
  }
}}
...
int h[7];
{
  ...
  for(i=0;i<6;i++){
    ...
    set[5]=5;
    result[z]=score_seven(first,set);
    z++;
    }
  }
...
int score_seven(first,set)  /* ¦^¶Ç¤À¨â°ô«áªºµû¤À(AI), 
